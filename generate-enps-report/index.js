...
const generateENPSReport = require("./controllers/generateENPSReport");
...
exports.handler = async (event, context, callback) => {
  ...
  try {
    ...
    for (i = 0; i < allOrganisations.length; i++) {
      await generateENPSReport(allOrganisations[i]._id);
    }
  } catch (err) {
    console.log(
      `Error in running generate enps report function at ${new Date()}`,
      err
    );
    callback(
      null,

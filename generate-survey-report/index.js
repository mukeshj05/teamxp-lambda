...
const generateSurveyReport = require("./controllers/generateSurveyReport");
...
exports.handler = async (event, context, callback) => {
  ...
  try {
    ...
    for (i = 0; i < allOrganisations.length; i++) {
      await generateSurveyReport(allOrganisations[i]._id);
    }
  } catch (err) {
    console.log(
      `Error in running generate survey report function at ${new Date()}`,
      err
    );
    callback(
      null,

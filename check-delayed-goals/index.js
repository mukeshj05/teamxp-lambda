...
exports.handler = async (event, context, callback) => {
  ...
  try {
    const goals = await Goal.updateMany(
      { active: true, endDate: { $lte: new Date() } },
      { status: 2 }
    );
  } catch (err) {
    console.log(
      `Error in updating delayed goals function at ${new Date()}`,
      err
    );
    callback(null, `Error in updating delayed goals function at ${new Date()}`);
  }
};
